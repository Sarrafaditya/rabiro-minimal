...
function CountUp({ target, inView }: { target: string; inView: boolean }) {
  ...
  const hasLetter = /[A-Za-z]/.test(target); // K, M, etc.

  useEffect(() => {
    if (!inView || hasLetter) return;
    let start = 0;
  ...
  }, [inView, num, hasLetter]);

  // Static display for values like $75K+
  if (hasLetter) {
    return <span>{inView ? target : prefix + "0" + suffix}</span>;
  }
    ...
    <section className="py-24" style={{ backgroundColor: "var(--bg-inverse)" }}>
      <div className="container-main">
        <div className="text-center mb-16">
          ...
          <h2
            className="font-syne font-extrabold text-[var(--text-inverse)] mt-4"
        ...
        >
          {STATS.map((stat, i) => (
            <motion.div
              ...
              style={{
                borderRight: i % 2 === 0 ? "1px solid #D0C9C0" : "none", // ← mobile 2-col fix
            ...
            >
              <p
                ...
                style={{
                  ...
                  fontSize: "clamp(28px, 5vw, 60px)", // ← responsive
                }}
              >
        ...
        <p
          ...
          style={{ color: "#666", maxWidth: 560, margin: "32px auto 0" }}
        >
